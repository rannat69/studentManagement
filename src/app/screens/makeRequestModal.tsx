...
const Modal: React.FC<ModalProps> = ({ isOpen, request, onClose, onSave }) => {
  ...
  const [unfilteredCoursesListState, setUnfilteredCoursesListState] = useState<
    Course[]
  >([]);
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      ...
      setUnfilteredCoursesListState(response.data);
  ...
  const handleChange = async (
    ...
    // if teacher change, filter courses
    if (name === "teacher_id") {
      // Get courses for teacher
      const fetchCoursesTeacher = async (teacher_id: number) => {
        const response = await axios.get(`/api/course_teacher/0/${teacher_id}`);
      ...
      const resFetchCoursesTeacher = await fetchCoursesTeacher(Number(value));

      console.log("fetchCoursesTeacher", resFetchCoursesTeacher);

      // Filter courseListState where resFetchCoursesTeacher.course_id = course_id
      const filteredCourses = unfilteredCoursesListState.filter((course: Course) =>
        resFetchCoursesTeacher?.some(
          (courseTeacher: any) => courseTeacher.course_id === course.id,
        ),
      );

      console.log("filteredCourses", filteredCourses);
      setCoursesListState(filteredCourses);
    }
  };
  ...
  const updateRequest = async (id: number, updatedData: Request) => {
    try {
      const response = await axios.put(`/api/request/${id}`, updatedData);
        ...
        <div className={styles.modalContentColumn}>
          ...
          <div className={styles.inputContainer}>
            ...
            <select
              name="student_id"
              onChange={handleChange}
              value={formData && formData.student_id ? formData.student_id : 0}
              className={styles.smallSelect}
            >
            ...
            <select
              name="teacher_id"
              onChange={handleChange}
              value={formData && formData.teacher_id ? formData.teacher_id : 0}
              className={styles.smallSelect}
            >
              <option value={0}>None</option>
          ...
          <div className={styles.inputContainer}>
            ...
            <select
              name="course_id"
              onChange={handleChange}
              value={formData && formData.course_id ? formData.course_id : 0}
              className={styles.smallSelect}
            >
              <option value={0}>None</option>
