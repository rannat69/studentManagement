...
export default function MatchStudentCourse() {
  ...
  const [studentListAvailUnfiltered, setStudentListAvailUnfiltered] = useState<
    Student[]
  >([]);
  ...
  const [studentTeacher, setStudentTeacher] = useState<StudentTeacher[]>([]);
  ...
  const fetchStudentCourseForSemester = async (
    ...
    // list of teachers per student
    response = await axios.get("/api/student_teacher/all");

    setStudentTeacher(response.data);
    // list of teachers
    response = await axios.get("/api/teacher/all");
  ...
  const handleAutoMatch = async () => {
    ...
    const response = await axios.get("/api/request/teacherWant");
    ...
    const responseCourseQualif = await axios.get("/api/course_qualif/all");
    ...
    const responseStudentQualif = await axios.get("/api/student_qualif/all");
    ...
    // get all course_qualif
    let courseAreaTable = [];
    const responseCourseArea = await axios.get("/api/course_area/all");
    ...
    const responseStudentArea = await axios.get("/api/student_area/all");
    ...
    for (const course of courseListNeeded) {
      ...
      const responseStudentCourse = await fetch(
        "/api/student_course/allForCourse",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className={styles.page2}>
      <div className={styles.pageTitle}>
        <h1>Match</h1>
        <h3>
          Drag available students to courses that need teaching assistants
        </h3>
                ...
                >
                  ...
                  <div className={styles.dropArea}>
                    ...
                    <ProgressBar
                      striped
                      variant={
                        course.ta_needed === course.ta_assigned
                          ? "success"
                          : "danger"
                      }
                      now={
                        ...
                        course.ta_assigned > 0
                          ? (course.ta_assigned / course.ta_needed) * 100
                          : 0
                      }
                      label={`${
                        course.ta_needed &&
                        ...
                        course.ta_assigned &&
                        course.ta_assigned > 0
                          ? course.ta_assigned.toString() +
                            "/" +
                            course.ta_needed.toString()
                          : 0
                      }`}
                    />
                    ...
                    <div className={styles.innerDropArea}>
                      {studentListAssigned.filter(
                        ...
                        : course.ta_needed > 0 && (
                            <div className={styles.emptyDropArea}>
                              Drop a student here
                            </div>
                          )}
                    </div>
          ...
          {areYouSure && (
            <div className={styles.modal}>
              <div className={styles.modalContent}>
                This will remove all students of all classes for the period{" "}
                {semester} {year}.<br /> Are you sure ?
                <div
                  className={styles.buttonClear}
