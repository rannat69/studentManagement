	...
	const handleSearchStudent = (property: string, searchTerm: string) => {
		...
		const filteredList = studentListStateUnfiltered.filter((student) => {
			...
			const matchesName = currentSearchName.length === 0 || (
				(student.l_name && student.l_name.toLowerCase().includes(currentSearchName.toLowerCase())) ||
				(student.f_names && student.f_names.toLowerCase().includes(currentSearchName.toLowerCase())) ||
				(student.unoff_name && student.unoff_name.toLowerCase().includes(currentSearchName.toLowerCase()))
			);

			const matchesYear = currentSearchYear === 0 || student.expected_grad_year === currentSearchYear;

			const matchesSemester = currentSearchSemester.length === 0 || student.expected_grad_semester === currentSearchSemester;
			...
			<div className={ styles.center }>
				<div className={ styles.header }>
					...
					<select
						...
						onChange={ (e) => handleSearchStudent("year", e.target.value) } className={ styles.select }>
					...
					<input type="text" className={ styles.input } placeholder="&#128269;   Search students ..." onChange={ (e) => handleSearchStudent("name", e.target.value) }></input>
							...
							{ studentListState &&
								studentListState.map((student) => (
									<tr
										...
										<td>{ student.l_name && student.l_name.length > 0 ? student.l_name : "-" }</td>
										<td>{ student.f_names && student.f_names.length > 0 ? student.f_names : "-" }</td>
										<td>{ student.unoff_name && student.unoff_name.length > 0 ? student.unoff_name : "-" }</td>
										<td>{ student.expected_grad_year }</td>
										<td>{ student.expected_grad_semester === "Spring" && <div className={ styles.semesterSpring }>{ student.expected_grad_semester }</div> }
											{ student.expected_grad_semester === "Summer" && <div className={ styles.semesterSummer }>{ student.expected_grad_semester }</div> }
											{ student.expected_grad_semester === "Fall" && <div className={ styles.semesterFall }>{ student.expected_grad_semester }</div> }
											{ student.expected_grad_semester === "Winter" && <div className={ styles.semesterWinter }>{ student.expected_grad_semester }</div> }</td>
